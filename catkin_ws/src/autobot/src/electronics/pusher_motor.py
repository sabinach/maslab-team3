...
# Cylinder pusher motor;
# Uses RIGHT motor from motor controller (shares with dispenser motor)
# To run: rosrun autobot pusher_motor.py
...
LEFT = (0, 1)    # (left motor, right motor)
RIGHT = (0, -1)
STOP = (0, 0)
...
class PusherMotor:
    ...
    def __init__(self):
        ...
        self.pub_dispenser_drive = rospy.Publisher('pusher_cmd', kitware.msg.DriveCMD, queue_size=10)
    ...
    def pusher_drive(self):
        """Publish drive commands to pusher motor"""
        pusher_drive_msg = kitware.msg.DriveCMD()
        pusher_drive_msg.l_speed, pusher_drive_msg.r_speed = self.drive_speed
        self.pub_dispenser_drive.publish(pusher_drive_msg)
    ...
    def set_dispenser_cmds(self, mappings):
        move_left = mappings["button_y"]
        move_right = mappings["button_a"]
